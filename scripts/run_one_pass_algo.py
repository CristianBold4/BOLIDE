...
def run_one_pass_algo(args: argparse.Namespace) -> None:
    # -- assertions
    assert (0 <= args.head_memory_perc <= 1 and 0 <= args.tail_memory_perc <= 1), "Error! hp and ht must be in [0, 1]"
    assert (0 <= args.p_sample_head <= 1 and 0 <= args.p_sample_tail <= 1), "Error! p_sample_head and p_sample_tail must be in [0, 1]"
    assert (0 <= args.aux_memory_perc <= 1), "Error! aux_memory_perc must be in [0, 1]"

    # -- read dataset info
    with open('utils/datasets_map.json', 'r') as f:
        dataset_info = json.load(f)[args.dataset_name]
    ...
    head_memory_budget = int(args.head_memory_perc * num_edges)
    tail_memory_budget = int(args.tail_memory_perc * num_edges)

    sample_root = (output_root / Path(f'node_sample_h{args.p_sample_head}_t{args.p_sample_tail}'))
    ...
    for trial in range(1, N_TRIALS+1):
        ...
        aux_memory_budget = int(args.aux_memory_perc * num_edges)

        exp_output_root = sample_root / Path(f'aux_b{args.aux_memory_perc}_hp{args.head_memory_perc}_tp{args.tail_memory_perc}') / Path(f'trial_{trial:02d}')
        ...
        params_info = {
            ...
            'num_nodes': num_nodes,
            'num_edges': num_edges,
            'p_sample_head': args.p_sample_head,
            'p_sample_tail': args.p_sample_tail,
            'aux_memory_perc': args.aux_memory_perc,
            'head_memory_perc': args.head_memory_perc,
            'tail_memory_perc': args.tail_memory_perc,
        ...
        print(f'\n ----- Running Triangle Sampler with aux_memory_perc: {args.aux_memory_perc} ----- \n')
        output_path_exp = exp_output_root / Path(f'exp_output_driver.txt')
        ...
        command = (f"{RUN_ONE_PASS_ALGO_EXECUTABLE_PATH} "
                   ...
                   f"{args.p_sample_head} " # p_h
                   f"{args.p_sample_tail} " # p_t
...
if __name__ == '__main__':
    ...
    parser.add_argument('--p_sample_head', '-psh', type=float, default=0.2, help='Probability of sampling nodes (NodeSampler).')
    parser.add_argument('--p_sample_tail', '-pst', type=float, default=0.005, help='Probability of sampling edges (NodeSampler).')
    parser.add_argument('--head_memory_perc', '-hp', type=float, default=0.1, help='Percentage of memory budget for head main sample.')
    parser.add_argument('--tail_memory_perc', '-tp', type=float, default=0.01, help='Percentage of memory budget for tail main sample.')
    parser.add_argument('--aux_memory_perc', '-ap', type=float, default=0.01, help='Percentage of memory budget for auxiliary samples (summed).')
